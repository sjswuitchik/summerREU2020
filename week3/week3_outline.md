...
## Step 1: Test out SAMtools command on trial data
...
Useful commands for this step that we covered in the intro to R virtualREU session are `library` (to load the tidyverse package), `mutate` (to make new columns with new values), `select` (to choose which columns to include), `rename` (to rename columns), and `write_delim` (to export the final files with the proper delimiter and file extension).  
